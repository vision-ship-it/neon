...
impl<const N: usize> HyperLogLogState<N> {
    pub fn measure(&self, item: &(impl Hash + ?Sized)) {
        tracing::info!("/neon/libs/metrics/src/hll.rs: Measuring item with hash");
    ...
    fn collect_into(
        ...
        self.take_sample()
            ...
            .try_for_each(|(hll_shard, val)| {
                if let Err(e) = CounterState::new(val as u64).collect_into(
                    &(),
                    ...
                    tracing::error!("/neon/libs/metrics/src/hll.rs: Error collecting into encoder: {}", e);
                    return Err(e);
                }
                Ok(())
            })
